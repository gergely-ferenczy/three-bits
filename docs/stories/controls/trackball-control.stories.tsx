...
/**
 * TrackballControl component wrapper for React Three Fiber.
 * Integrates the vanilla Three.js TrackballControl with R3F.
 */
interface TrackballControlDemoProps {
  showInfo: boolean;
...
const TrackballControlDemo: React.FC<TrackballControlDemoProps> = ({
  rotationSpeed = 1,
  truckSpeed = 1,
  zoomDollySpeed = 1,
  minDistance = 1,
  ...
  const controlRef = useRef<TrackballControl | null>(null);
  ...
  useEffect(() => {
    ...
    const control = new TrackballControl(camera, options);
 ...
 */
const TrackballControlStory = (props: TrackballControlDemoProps) => {
...
// Storybook metadata
const meta: Meta<typeof TrackballControlStory> = {
  title: 'Controls/TrackballControl',
  component: TrackballControlStory,
  parameters: {
    layout: 'fullscreen',
  },
  argTypes: {
...
export default meta;
type Story = StoryObj<typeof TrackballControlStory>;
